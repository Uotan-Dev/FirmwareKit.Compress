...
namespace FirmwareKit.Compress.Internal.Zopfli
{
    ...
    */
    sealed class OutputSink
    {
        ...
        public void FlushCompleted()
        {
            ...
            if (n <= 0) return;
            Polyfill.WriteTo(_out, Polyfill.AsSpan(_buf).Slice(0, n));
        ...
        public void Finish()
        {
            if (_buf.Count > 0)
            {
                Polyfill.WriteTo(_out, Polyfill.AsSpan(_buf));
                _flushed += _buf.Count;
                _buf.Clear();
            }
            _out.Flush();
    ...
    class ZopfliLZ77Store
    {
        ...
        public ushort[] litlens = null!;  /* Lit or len. */
        public ushort[] dists = null!;  /* If 0: indicates literal in corresponding litlens,
      if > 0: length in corresponding litlens, this is the distance. */
        public ulong size;

        public byte[] data = null!;  /* original data */
        public ulong[] pos = null!;  /* position in data where this LZ77 command begins */

        public ushort[] ll_symbol = null!;
        public ushort[] d_symbol = null!;
        ...
        public ulong[] ll_counts = Array.Empty<ulong>();
        public ulong[] d_counts = Array.Empty<ulong>();
    ...
    {
        /* These are allocated lazily (non-null after the first ZopfliAllocLongestMatchCache). */
        public ushort[] length = null!;
        public ushort[] dist = null!;
        public byte[] sublen = null!;
    }
        ...
        */
                             ...
                             ushort[] sublen)
        {
            ...
            /* Hoist the per-position base index and array reference out of the loop,
               and fill each length-range with a vectorized Polyfill.Fill instead of a
            ...
            for (int j = 0; j < ZOPFLI_CACHE_LENGTH; j++)
            {
                ...
                if (count > 0) Polyfill.Fill(sublen, dist, prevlength, count);
    ...
    */
        ...
        public void ZopfliResetHash()
        {
            ...
            Polyfill.Fill(head, -1 /* -1 indicates no head so far. */);
            ...
            Polyfill.Fill<short>(hashval, -1 /* -1 indicates no head so far. */);
            Array.Clear(same, 0, same.Length);

            val = 0;
            val2 = 0;
            Polyfill.Fill(head2, -1 /* -1 indicates no head so far. */);
            Polyfill.Fill<short>(hashval2, -1 /* -1 indicates no head so far. */);
        }
        /*
    ...
    {
        public ZopfliLZ77Store lz77 = null!;
        public ulong start;
        public ulong end;
    }
