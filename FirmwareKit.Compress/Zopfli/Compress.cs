...
{
    internal partial class Compress
    {
        /*
        ...
        */
        static int GetMatch(int scan, int match, int end, byte[] array)
        {
            ...
            while (scan < safeEnd)
            {
                ulong x = MemoryMarshal.Read<ulong>(array.AsSpan(scan, 8));
                ulong y = MemoryMarshal.Read<ulong>(array.AsSpan(match, 8));
                if (x != y)
                {
                    return scan + (Polyfill.TrailingZeroCount(x ^ y) >> 3);
                }
                scan += 8; match += 8;
            }
        ...
        */
        ...
        {
            Polyfill.Fill(ll_lengths, 8u, 0, 144);
            Polyfill.Fill(ll_lengths, 9u, 144, 256 - 144);
            Polyfill.Fill(ll_lengths, 7u, 256, 280 - 256);
            Polyfill.Fill(ll_lengths, 8u, 280, 288 - 280);
            Polyfill.Fill(d_lengths, 5u, 0, 32);
        }
        ...
        */
        static void ZopfliDeflatePart(int btype, bool final, byte[] InFile, int instart, int inend, ref int bp, OutputSink OutFilePart)
        {
            ...
            {
                /* blocksplitting=0：不切分，对整段输入做一次最优 LZ77。 */
                /* blocksplitting=0: no splitting, run one optimal LZ77 pass over the whole input. */
                ZopfliBlockState s = new ZopfliBlockState(instart, inend, 1);
                ZopfliLZ77Store store = new ZopfliLZ77Store(InFile);
                ZopfliLZ77Optimal(s, InFile, instart, inend, Globals.numiterations, store);
                ZopfliAppendLZ77Store(store, lz77);
            }
